...
export default function AccountManagement() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await axios.get(`https://abmscapstone2024.azurewebsites.net/api/v1/account/get?buildingId=${item?.bid}&role=1`, {
                    timeout:100000
                });
            ...
            <SafeAreaView style={{flex:1}}>
                <ScrollView style={{ paddingHorizontal: 30, paddingVertical: 30,flex:1 }}>
                    <View style={{ marginBottom: 20 }}>
                        ...
                        <Text>Thông tin tài khoản của quản lý chung cư tòa <Text style={{fontWeight: 'bold', fontSize: SIZES.medium}}>{building?.name}</Text></Text>
                    </View>
                    <Link href={`/web/Admin/buildings/accounts/create?bid=${item?.bid}`}>
                    ...
                    {!filteredRequests.length ? <Text style={{marginBottom:10, fontSize:18,fontWeight:'600'}}>Chưa có dữ liệu</Text>:
                          <TableComponent headers={headers}>
                            ...
                            <TableRow >
                                ...
                                <Cell>
                                    <Link href={`/web/Admin/buildings/accounts/${item.id}`}
                                    >
