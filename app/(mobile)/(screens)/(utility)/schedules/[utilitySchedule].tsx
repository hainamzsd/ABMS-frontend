...
import { SafeAreaView, StyleSheet, View, ScrollView, FlatList, Pressable } from "react-native";
...
export default function Schedule() {
  ...
  const isButtonDisabled = selectedDate === 0 || !selectedDate;
  return (
    <>
      ...
      <SafeAreaView style={{ backgroundColor: theme.background, flex: 1, justifyContent: 'space-between' }}>
        <View style={{ flex: 1 }}>
          ...
          <View style={{ marginTop: 30, marginHorizontal: 26, flex: 1 }}>
            <Text style={{ fontWeight: 'bold', fontSize: 24 }}>{t("Choose hour")}</Text>
            <FlatList
              data={data}
              renderItem={({ item }) =>
                <Pressable
                  ...
                  style={[styles.slot,
                  {
                    backgroundColor: selectedSlot === item.id ? theme.primary : 'transparent',
        ...
        <View style={{
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                pathname: `/(mobile)/(screens)/(utility)/schedules/tickets/utilityTicket`,
                params: selectedDate,
              })}
            style={[
              {
...
const styles = StyleSheet.create({
  ...
  slot: {
    ...
    width: '45%', marginTop: 10, marginRight: 10
  },
  button: {
    padding: 20,
    borderRadius: 20,
